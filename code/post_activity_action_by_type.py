...
#bên post đã làm được delete year and month bỏ drop constraint, bên này vẫn giữ constraint để tránh bị trùng
 # proxies = {
    ...
    conn = pyodbc.connect('Driver={SQL Server};'
                      ...
                      'Trusted_Connection=yes;')
    cursor = conn.cursor()
    cursor.execute("select post.id from post left join post_activity_by_action_type on post.id = post_activity_by_action_type.id where post_activity_by_action_type.id is null order by date desc")
